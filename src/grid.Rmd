...
```{r}
training <- function(x){
  trimmed <- x[-c(1,4,8:10)] %>%  # Removing unnecessary offline variables
    mutate(diffMAX = (max(RSSI)-RSSI)) %>% 
    group_by(posX, posY, MAC) %>% 
    mutate(avg_RSSI = mean(RSSI)) %>% 
    mutate(avg_diffMAX = mean(diffMAX)) %>%
    mutate(rp_mag = magnitude(posX, posY)) %>% 
    mutate(ap_mag = magnitude(ap_x,ap_y)) %>% 
    mutate(rp_ap_mag = euclidean(rp_mag, ap_mag))
...
```{r}
cos_mat <- as.matrix(avg_off$rp_mag) 
cos_mat[cos_mat(2] = avg_off$ap_mag
cos_mat
```


```{r}
ggplot(data = avg_off, aes(dist, RSSI, group=posXY, colour = MAC)) +
  # geom_line()
  geom_boxplot()
```
...
```{r}
temp <- mutate(avg_off, calc_dist = round(10^(diffMAX-32.44-20 * log10(frequency)/20), digits = 3)) 
temp
```
...
fingerprints <- function(x){
  reduced <- x %>% select(-c(RSSI, diffMAX)) %>% distinct()
  return(reduced)
}
```
...
```{r}
lmod <- lm(avg_RSSI ~ dist,data = off_fp)
summary(lmod)
```


```{r}
off_fp %>%                                        # Specify data frame
  group_by(posXY) %>%                         # Specify group indicator
  summarise_at(vars(avg_RSSI),              # Specify column
               list(name = mean))               # Specify function

# aggregate(x = off_fp$posXY,                # Specify data column
#           by = list(off_fp$avg_RSSI),              # Specify group indicator
#           FUN = mean)                           # Specify function (i.e. mean)
# mutate(off_fp, var = var())
```


```{r}
ggplot(data= off_fp)+
  geom_point(aes(dist, avg_RSSI, group=posXY, colour=MAC ))
```
