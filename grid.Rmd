...
c_min <- setNames(aggregate(new_off$c, by = list(new_off$ap_mac), min), c("ap_mac", "min_dist"))
ap_dist <- merge(ap_locs, c_max, by="ap_mac")
ap_dist <- merge(ap_dist, c_min, by="ap_mac")
ap_dist
```
...
```{r}
split.data.frame(new_off, f = list(new_off$posX, new_off$posY))
```
